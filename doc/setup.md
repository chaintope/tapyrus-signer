...
```
tapyrus-setup createvss -public_key=1:<node_public_key[1]> -public_key=2:<node_public_key[2]> ... -public_key=n:<node_public_key[n]> -private_key=<node_private_key[i]> -nonce=<networkid> -threshold=<t>
...
- `node_public_key[]` is a sorted array of public keys of signers and input args `j:node_public_key[j]` (j = 1, 2, ..., n) is a index and public key generated by Signer[j] in Step 2.
...
```
tapyrus-setup aggregate -vss=1:<node_vss[1, i]> -vss=2:<node_vss[2, i]> ... -vss=n:<node_vss[n, i]> -private_key=<node_private_key[i]>
...
To generate `local_sig`, Signer[i] need at least t valid `block_vss[j, i]`s. In the following steps, we suppose that Signer[1], Signer[2], ... Signer[t] are honest signers providing a valid vss and Signer[i] got `block_vss[j, i]`(j = 1, 2, 3, ..., t).

```
tapyrus-setup sign -node_vss=1:<node_vss[1, i]> -node_vss=2:<node_vss[2, i]> ... -node_vss=t:<node_vss[t, i]> -block_vss=1:<block_vss[1, i]> -block_vss=2:<block_vss[2, i]> ... -block_vss=t:<block_vss[t, i]>   -aggregated_public_key=<aggregated_public_key> -nodesecret=<node_secret[i]> -private_key=<node_private_key[i]> -block=<block>

output: <local_sig[i]>
```

- `node_vss[j, i]` (j = 1, 2, ..., t) are the node VSSs generated by Step 3.
- `block_vss[j, i]` (j = 1, 2, ..., t) are the block VSSs generated by Step 7.
- `node_private_key[i]` is the private key of Signer[i], generated in Step 2.
...
In above list, `node_private_key` and `node_secret` must be treated as secret.
